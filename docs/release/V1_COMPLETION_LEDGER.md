...
| Isolated Supabase backend | AWAITING_OWNER | Chrome organization `oudelzlwsnqgeolvtrsz` logged in (Free); `QuestLife V1 Candidate` creation prepared, new database password must be entered by owner; existing production `gtlknzltzntfltgjvgxx` untouched |
...
| Auth and bidirectional sync | IMPLEMENTED, acceptance pending | Existing Sync V2; hosted RLS/OTP/concurrency/cross-device checks remain |
...
| Standalone iPhone installation | AWAITING_OWNER | EAS login verified; iOS simulator non-development cloud build `7197f964-e1c8-4741-bda7-44ccd560ab9f` FINISHED; this is not a physical-iPhone package. Signing and UDID still required |
| Same-version Web candidate | VERIFIED_REMOTE intermediate deployment, final rebuild pending | `https://questlife-v1-release.vercel.app` returns HTTP200 without preview SSO; deployment `dpl_2NXp3iJTHscYZTvZVfrYCo6f2cgq`. Same repo/branch, intermediate dirty snapshot. Owner production untouched. Final commit still needs rebuild/deploy |
...
1. Finish the prepared free Supabase candidate creation using a private database
   password, then complete CLI authorization. Existing production remains untouched.
...
- Remaining product-code gap: no validated JSON file import/restore flow.
  Export is not a full device backup (no credentials, raw Health repository,
  permission state or sync journal). Do not claim import acceptance.
