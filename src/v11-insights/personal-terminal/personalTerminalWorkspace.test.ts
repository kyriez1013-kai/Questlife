...
const oneMonth = buildPersonalTerminalRangeViewData(marketOne, { kind: 'preset', preset: '1M' }, now, null);
assert.equal(fourDay.observations.length, 4);
assert.equal(nineDay.observations.length, 9);
assert.equal(oneMonth.line.length, 10);
assert.equal(oneMonth.line[oneMonth.line.length - 1].value, 10);
