...
export default forwardRef<InsightsV3ChartHandle,{model:QuestLifeChartModelV1}>(({model},ref)=>{
  ...
  // Read-only callbacks may rerender the parent; identical values must not reset zoom.
  useEffect(()=>{if(loaded){setError(false);setSelection(null);send({type:'model',model:JSON.parse(signature)});}},[loaded,signature]);
