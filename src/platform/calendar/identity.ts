...
export function externalCommitmentIdentity(row: ExternalCommitmentV1): string {
  ...
  if (row.provider && row.providerCalendarId && row.providerEventId) return JSON.stringify([row.provider,row.providerCalendarId,row.providerEventId,row.startAt]);
  return JSON.stringify([row.platform ?? 'device_local',row.calendarId,row.externalEventId,row.startAt]);
}
export function uniqueCommitments(rows: ExternalCommitmentV1[]) {
  return [...new Map(rows.map(row=>[externalCommitmentIdentity(row),row])).values()];
}
