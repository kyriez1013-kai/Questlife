import type { NotificationKind, NotificationRequest, NotificationService, PermissionState, QuickActionIntent, QuickActionService } from '../contracts';
...
export function isNotificationKind(kind: unknown): kind is NotificationKind {
  return typeof kind==='string' && ['accepted_block','decision_followup','morning_state','end_of_day','skill_reminder'].includes(kind);
}
export function createNotificationService(driver: NotificationDriver, now = () => Date.now()): NotificationService {
  const validate = async (request: NotificationRequest) => {
    ...
    if(Date.parse(request.at)<=now()) throw new Error('notification_invalid_request');
  };
  ...
  return {permission:()=>driver.permission(false),requestPermission:()=>driver.permission(true),schedule,cancel,reschedule:async request=>{ await validate(request); await cancel(request.id); if(Date.parse(request.at)<=now())throw new Error('notification_invalid_request'); return driver.schedule(request); },subscribe:listener=>driver.subscribe(listener),getPushToken:id=>driver.token(id)};
}
export function parseNotificationIntent(data: Record<string,unknown>, action: string, notificationId: string): QuickActionIntent | null {
  ...
  if(!isNotificationKind(data.kind)) return null;
  if(!['START','DONE','SNOOZE','SKIP','OPEN'].includes(action)) return null;
  return {action:action as QuickActionIntent['action'],kind:data.kind,entityId:typeof data.entityId === 'string' ? data.entityId : undefined,notificationId};
}
