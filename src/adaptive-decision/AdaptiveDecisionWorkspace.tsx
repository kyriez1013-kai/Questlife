...
import {
  buildDecisionSurfacePresentation,
  type DecisionSurfacePlanChangeV2,
} from './decisionSurfacePresentation';
...
export default function AdaptiveDecisionWorkspace({
  ...
  const planChanges = presentation.primaryAction
    ? [...presentation.primaryAction.planChanges, ...presentation.primaryAction.protectedItems]
    : [];
  const rootVariables = {
  ...
  const renderPlanChange = (change: DecisionSurfacePlanChangeV2) => (
  ...
  const evidenceLink = (labelKey = 'adaptiveSurfaceFullEvidence') => (
    <WebPressable
      accessibilityRole="button"
      onPress={() => setEvidenceOpen(true)}
      uiClass="adl2-evidence-link"
    >
      <WebText uiClass="adl2-evidence-link-text">{copy(labelKey)}</WebText>
  ...
  const page = (
    <WebView uiClass="adl2-page">
      ...
      {applied && presentation.primaryAction ? (
        <WebView dataSet={{ 'adl-state': 'receipt' }} uiClass="adl2-receipt">
          <WebView uiClass="adl2-receipt-heading">
            <WebView uiClass="adl2-receipt-mark">
              <V11RebaselineIcon color={theme.text.primary} name="update" size={20} />
            </WebView>
            <WebView uiClass="adl2-receipt-copy">
              <WebText uiClass="adl2-recommendation-label">{copy('adaptiveSurfaceApplied')}</WebText>
              <WebText accessibilityRole="header" uiClass="adl2-receipt-title">
                {presentation.primaryAction.title}
              </WebText>
            </WebView>
          </WebView>
          <WebView uiClass="adl2-plan-diff">{planChanges.map(renderPlanChange)}</WebView>
          <WebText uiClass="adl2-receipt-stability">{copy('adaptiveSurfaceOtherPlansUnchanged')}</WebText>
        ...
        <>
          ...
          {presentation.contextItems.length > 0 ? (
          ...
          {presentation.isAbstained ? (
            <WebView dataSet={{ 'adl-state': 'abstained' }} uiClass="adl2-safety-layout">
              <WebView uiClass="adl2-safety-heading">
                <WebView uiClass="adl2-safety-mark">
                  <V11RebaselineIcon color={theme.text.primary} name="activity" size={20} />
                </WebView>
              ...
              {evidenceLink('adaptiveSurfaceViewReadContext')}
            </WebView>
          ) : null}

          {!presentation.isAbstained && episode.status !== 'NEEDS_INPUT' && presentation.primaryAction ? (
            <WebView dataSet={{ 'adl-state': 'resolved' }} uiClass="adl2-decision-layout">
              <WebView uiClass="adl2-recommendation">
                ...
                <WebView uiClass="adl2-plan-diff">{planChanges.map(renderPlanChange)}</WebView>
                {presentation.primaryAction.basisNote ? (
                  <WebText uiClass="adl2-basis-note">{presentation.primaryAction.basisNote}</WebText>
                ) : null}
                <WebText uiClass="adl2-recommendation-description">
                  {presentation.primaryAction.description}
                </WebText>
              </WebView>
              ...
              {canApply ? (
                <WebView uiClass="adl2-apply-wrap">
                  <V11Pill
                    ...
                    contentStyle={{ alignItems: 'center', justifyContent: 'center', paddingHorizontal: 20 }}
                    height={52}
                  ...
                  {presentation.alternatives.map((alternative) => (
                    ...
                    >
                      <WebView uiClass="adl2-alternative-copy">
                        ...
                        <WebText numberOfLines={2} uiClass="adl2-alternative-effect">
                          {alternative.exactEffect}
                        </WebText>
                      </WebView>
                      <V11RebaselineIcon color={theme.text.secondary} name="arrow" size={16} />
                    </WebPressable>
              ...
              <WebView uiClass="adl2-why-block">
                ...
                {presentation.primaryAction.reasonLines.map((line) => (
                  <WebText key={line} uiClass="adl2-reason-text">{line}</WebText>
                ))}
                {evidenceLink()}
              </WebView>
            </WebView>
