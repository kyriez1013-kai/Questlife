...
assert.match(cue.text, /appears repeatedly/);
assert.match(cue.detail || '', /counterexamples remain/);
assert.doesNotMatch([cue.text, cue.detail, cue.evidence].filter(Boolean).join(' '), /causes|will|recommend/);
