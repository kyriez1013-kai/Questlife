...
export function V11ComposerAction(p: Props<'V11ComposerAction'>) { return <Pressable accessibilityRole="button" accessibilityLabel={p.label} accessibilityState={{ disabled: p.disabled || p.loading, busy: p.loading }} disabled={p.disabled || p.loading} onPress={p.onPress} style={({pressed})=>[hit, { width: 44, alignItems:'center', backgroundColor: p.theme.control.neutralAction, opacity:p.disabled?0.45:pressed?0.75:1 }]}>{p.loading ? <ActivityIndicator color={p.theme.control.neutralActionText} /> : p.children}</Pressable>; }
