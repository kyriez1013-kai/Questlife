...
export const sessionStorage = {
  async getItem(key: string) {
    ...
    if (
      ...
      throw new Error("session_storage_invalid");
    const chunks = await Promise.all(
      Array.from({ length: manifest.count }, (_, i) =>
        SecureStore.getItemAsync(`${safe(key)}.${manifest.generation}.${i}`),
      ),
    );
    if (chunks.some((value) => value === null))
      throw new Error("session_storage_incomplete");
    return chunks.join("");
  },
  async setItem(key: string, value: string) {
    const previous = await SecureStore.getItemAsync(safe(key));
    const generation = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
    // At most 1536 UTF-8 bytes even for non-Latin profile metadata.
    const chunks = value.match(/[\s\S]{1,512}/g) ?? [""];
    for (let i = 0; i < chunks.length; i++)
      await SecureStore.setItemAsync(
        `${safe(key)}.${generation}.${i}`,
        chunks[i],
      );
    await SecureStore.setItemAsync(
      safe(key),
