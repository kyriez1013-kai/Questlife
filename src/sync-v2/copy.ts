const messages = {
  zh: {
    ...
    request: "发送登录邮件",
    verify: "验证并登录",
    sent: "登录邮件已请求。请在发起请求的同一浏览器或设备打开邮件中的链接；若邮件包含验证码，也可在下方输入。未收到请检查垃圾邮件或稍后重试。",
    linkFailed: "登录链接未能完成验证。请在本设备重新请求邮件，使用最新链接；不要把链接转发给他人。",
    signOut: "退出登录并保留本地数据",
    clearLocal: "清除本机记录副本",
    cancel: "取消",
    clearWarning:
  ...
  en: {
    ...
    email: "Email",
    otp: "Email code",
    request: "Send sign-in email",
    verify: "Verify and sign in",
    sent: "Sign-in email requested. Open its link in the same browser or device that made this request. If the email includes a code, you can enter it below. Check spam or retry later if it does not arrive.",
    linkFailed: "The sign-in link could not be verified. Request a new email on this device and use its latest link. Do not forward the link.",
