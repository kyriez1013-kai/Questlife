...
export const authConfigured = () => !!url && /^https:\/\//.test(url) && !!key && !key.startsWith('sb_secret_');
...
const identity = (session: { user: { id: string; email?: string } } | null): IdentitySession | null => session ? { userId: session.user.id, email: session.user.email } : null;
export const authService = new AuthService({
  ...
  subscribe(listener) {
    ...
    const { data } = supabaseClient().auth.onAuthStateChange((_event, session) => listener(identity(session)));
