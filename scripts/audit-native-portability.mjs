...
function resolveNative(from, specifier) {
  if (!specifier.startsWith('.')) return null;
  const base = path.resolve(path.dirname(from), specifier);
  for (const suffix of ['.native.tsx', '.native.ts', '.tsx', '.ts', '.js', '/index.native.tsx', '/index.tsx', '/index.ts', '']) {
...
function visit(file) {
  ...
  function node(n) {
    if (ts.isImportDeclaration(n) && !n.importClause?.isType && ts.isStringLiteral(n.moduleSpecifier)) visit(resolveNative(file, n.moduleSpecifier.text));
    if (ts.isExportDeclaration(n) && !n.isType && n.moduleSpecifier && ts.isStringLiteral(n.moduleSpecifier)) visit(resolveNative(file, n.moduleSpecifier.text));
    ts.forEachChild(n, node);
  }
  node(source);
}
visit(path.join(root, 'index.ts'));
const patterns = {
...
fs.writeFileSync(path.join(out, 'NATIVE_PORTABILITY_AUDIT.json'), JSON.stringify({ version: 1, counts, scannedFiles: files.length, nativeGraph: [...reachable].map(f => path.relative(root, f)).sort(), issues }, null, 2) + '\n');
